...
Socket::Socket(): epoll_listener(-1), socket_fd(-1){}

Socket::Socket(int _socket_fd, int _epoll_listener) : socket_fd(_socket_fd),\
...
Socket::Socket(Socket&& tmp) : Socket()
{
	swap(*this, tmp);
}


bool Socket::has_no_task()
{
...
void Socket::write()
{
	...
	if (write_queue.empty()) // if all is writen, remove from waiting for write events
	{
		epoll_event event;
		event.data.fd = socket_fd;
		event.events = EPOLLIN;
