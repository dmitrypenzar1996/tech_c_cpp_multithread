...
int main(int argNum, char** args) {
    ...
    if (epoll_ctl(epoll_listener, EPOLL_CTL_ADD, server_socket, &event) < 0) {
    ...
    while (true) {
        int event_num = epoll_wait(epoll_listener, epoll_event_arr, MAX_EPOLL_EVENTS, -1);
            ...
            try {
                ...
                if (sfd == server_socket) {
                    ...
                    while (true) {
                        ...
                        new_connection = accept(server_socket, &c_addr, &c_len);
                        if (new_connection < 0) {
                    ...
                    if (epoll_event_arr[i].events & EPOLLIN) {
                        ...
                        while (true) {
                            ...
                            if (count < 0) {
                                if (errno != EAGAIN) {
                                    fprintf(stderr, "Error %d, %s at socket %d", errno,
                                        strerror(errno), sfd);
                                    is_closed = 1;
                                }
                                break;
