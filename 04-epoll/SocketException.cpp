...
SocketException::SocketException(SocketException&& temp)
    : SocketException() {
    swap(*this, temp);
}
...
SocketException::SocketException(int _errcode, int socket_fd) {
    ...
    snprintf(msg, MAX_MSG_SIZE, "SocketError, error code %d : %s", errcode,
        strerror(errcode));
}
...
SocketException::~SocketException() { delete[] msg; }

const char* SocketException::what() const throw() { return msg; }
